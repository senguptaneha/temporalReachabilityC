...
bool intervalsOverlap(std::vector<Interval*> l1, std::vector<Interval*> l2, int startTime, int endTime){
	int i = 0, j = 0, lastEndTime = -1, firstStartTime = endTime + 1;
	...
	if ((firstStartTime <= startTime) && (lastEndTime >= endTime)) return true;
	return false;
}
			...
			for (int i = 0; i < n->getNumEdges(true); i++){
				...
				for (int j = 0; j < intervalS[u].size(); j++){
					...
					if (!subFlag){
						...
						if (intervalsOverlap(intervalS[v], intervalD[v], startTime, endTime)){
							answer = 1;
							break;
						}
