...
struct bfsNode{
	int vertexId;
	struct bfsNode *next;
} *queue = NULL, *rear = NULL, *queueR = NULL, *rearR = NULL;
...
int dequeueR(){
	if ((queueR == NULL) && (rearR == NULL)){
		return -1;	//empty queue
	}
	struct bfsNode *temp = queueR;
	queueR = queueR->next;
	if (queueR == NULL) rearR = NULL;
	int v = temp->vertexId;
	free(temp);
...
int bbfs(int s, int d){
	int i,j,w;
	for (i=0; i<nVertices;i++){
		graph[i].color = 0;
		graphR[i].color = 0;
	}
	int qsizeS = 0, qsizeD = 0, answer = 0;
	enqueue(s); enqueueR(d);
	graph[s].color = 1; graphR[d].color = 1;
	qsizeS = 1; qsizeD = 1;

	for (w = 0; w < walkLength; w++){
		//expand source frontier
		int tempQS = qsizeS, tempQD = qsizeD;
		for (i = 0; i < tempQS; i++){
			int u = dequeue();
			qsizeS -= 1;
			for (j = 0; j< graph[u].numNbrs; j++){
		...
		for (i = 0; i < tempQD; i++){
			int u = dequeueR();
			qsizeD -= 1;
			for (j = 0; j < graphR[u].numNbrs; j++){
				int v = graphR[u].adj[j].vertexId;
				if (graphR[v].color == 0){
					graphR[v].color = graphR[u].color + 1;
					enqueueR(v);
					qsizeD += 1;
				}
				if (v == s){
	...
	if (answer != 1){
		for (i = 0; i < nVertices; i++){
			if ((graph[i].color > 0) && (graphR[i].color > 0)){
				answer = 1;
				break;
			}
		}
...
void processQueryBBFS(char *str);
...
/*	for (i = 0; i < 10;i++){
	...
	printf("PreprocessTime = %lf", 1000*preProcessTime);
	walkLength = c2*d;
...
void processQueryBBFS(char *str){
	...
	if ((trueAns>0) && (walkAns>0)) tp++;
	else if ((trueAns>0) && (walkAns==0)) fn++;
	else if ((trueAns==0) && (walkAns>0)) fp++;
